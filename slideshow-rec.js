...
(function () {
  ...
  function bgmForSlide(idx) {
    ...
    if (idx <= 0) return { url: D.stage_introUrl, lkey: D.stage_introLocal, key: 'intro' };
    if (idx >= 1 && idx <= total) {
      const sc = scenes[idx - 1] || {};
      if (sc.bgmUrl || sc.bgmLocal) return { url: sc.bgmUrl, lkey: sc.bgmLocal, key: 'sc' + idx };
      return { url: D.stage_transitionUrl, lkey: D.stage_transitionLocal, key: 'transition' };   // 막에 배경음악이 없으면 전환 음악을 깔아 줌
    }
    return { url: D.stage_endingUrl || D.curtainBgmUrl, lkey: D.stage_endingLocal || D.curtainBgmLocal, key: 'ending' };
  }
